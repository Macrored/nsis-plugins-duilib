...
void CRenderEngine::DrawGradient(HDC hDC, const RECT& rc, DWORD dwFirst, DWORD dwSecond, bool bVertical, int nSteps)
{
    ...
    if( lpGradientFill != NULL ) 
    {
        TRIVERTEX triv[2] = 
        {
            { rcPaint.left, rcPaint.top, (COLOR16)(GetBValue(dwFirst) << 8), (COLOR16)(GetGValue(dwFirst) << 8), 
			(COLOR16)(GetRValue(dwFirst) << 8), (COLOR16)0xFF00 },
            { rcPaint.right, rcPaint.bottom, (COLOR16)(GetBValue(dwSecond) << 8), (COLOR16)(GetGValue(dwSecond) << 8),
			(COLOR16)(GetRValue(dwSecond) << 8), (COLOR16)0xFF00 }
        };
